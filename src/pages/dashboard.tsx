...
function DashboardPage() {
  ...
  const electionStat = useSWR<PaginatedInterface<ElectionInterface>>(
    () => `/elections`,
    () => getElections({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Elections',
      result: electionStat?.data?.totalCount,
      isLoading: electionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
