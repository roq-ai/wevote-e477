...
import { getElectionById, updateElectionById } from 'apiSdk/elections';
...
function ElectionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ElectionInterface>(
    () => (id ? `/elections/${id}` : null),
    () => getElectionById(id),
  );
  ...
  const handleSubmit = async (values: ElectionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateElectionById(id, values);
      mutate(updated);
      resetForm();
